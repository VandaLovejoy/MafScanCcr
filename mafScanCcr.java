...
public class mafScanCcr {
	...
	public static synchronized void main(String[] Args) throws IOException {
		...
		for (int i = 0 ; i != Args.length ; i++ ) {
			...
			else if ( Args[ i ].equals("-ssz") ) { // step size
				SSZ = Double.parseDouble( Args[ i+1 ] )	; 
				i++ ; 
			}
			else if ( Args[ i ].equals("-sszr") ) { // step size
				SSZR = Double.parseDouble( Args[ i+1 ] )	; 
				i++ ; 
			}
			else if ( Args[ i ].equals("-rnaz") ) { // step size
				RNAZ = Double.parseDouble( Args[ i+1 ] )	; 
				i++ ; 
			}
	...
	private static synchronized boolean SplitNfold (String[][] MafTab, char[][] AlnTab) throws IOException {
		...
		for (int step = 0 ; step < AlnTab[0].length-WINDOW+STEP-1 ; step = step+STEP) {
			...
			//>>>>>>> add FILTER_ID <<<<<<<<
			ScanItFast Block = new ScanItFast( step, MafTab, AlnTab, Path, coordTab, WINDOW, STEP, GAPS, SSZBINARY, RNZBINARY, VERBOSE, PRINTALL );
            Block.setSsz( SSZ );
            Block.setSszR( SSZR );
            Block.setRnaz( RNAZ );
