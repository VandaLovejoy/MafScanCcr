...
public class ScanItFast implements Runnable {
	...
	private int goodSeqs, iterate, random, step, STEP, WINDOW, GAPS, retainedColumns;
	...
	ScanItFast(int step, String[][] MafTab, char[][] AlnTab, String Path, 
			 ...
			 String SSZBINARY, String RNZBINARY, boolean VERBOSE, boolean PRINTALL ) { 
		this.step = step ; 
		this.MafTab = MafTab ; 
		this.AlnTab = AlnTab ; 
		this.Path = Path ; 
...
//			AlnRC.delete() ;
		...
		if ( stats[0] <= 85 ) { // 85% sequence identity cut-off for RNAz
			...
			//write bed and rename alignment
				...
				// delete low scoring alignments
				if ( Double.parseDouble( RnazOutTab[ 4 ] ) < RNAZ_THRESHOLD )  {
					Aln.delete() ;
					if (PRINTALL) {
						System.out.println( FinalBedFile.replaceAll("_","\t")) ;
					}
...
*/							}
    ...
    public void setRnaz(double newValue){
        RNAZ_THRESHOLD = newValue;
    }
}
